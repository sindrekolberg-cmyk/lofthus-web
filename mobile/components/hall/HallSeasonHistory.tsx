...
import { HallMonthWinners } from "./HallMonthWinners";
  ...
  return (
    <View>
      {seasons.map((row) => {
        ...
        const months = data.monthly.filter((item) => item.season === row.season);
        return (
          <View key={row.season} style={styles.card}>
            ...
            {cup?.winner ? (
              <View style={styles.cup}>
                <HallCupTitle cup={cup} />
              </View>
            ) : null}
            <HallMonthWinners months={months} />
          </View>
        );
      })}
    </View>
